...
typedef enum MailHede MailHede;
...
typedef enum MailLdas MailLdas;
...
typedef enum MailQqcl MailQqcl;
...
typedef enum MailMvcl MailMvcl;
...
typedef enum MailExut MailExut;
...
typedef enum MailCclr MailCclr;
...
typedef enum MailE9xp MailE9xp;
...
typedef enum MailEnrq MailEnrq;
...
typedef enum MailErut MailErut;
...
enum MailEtoc {
	...
	MailEtocMobileMe = 'etit' /* MobileMe */
};
typedef enum MailEtoc MailEtoc;
...
@property (copy, readonly) MailText *content;  // Contents of an email message
...
// A Mail account for receiving messages (POP/IMAP/MobileMe). To create a new receiving account, use the 'pop account', 'imap account', and 'MobileMe account' objects
...
// A MobileMe email account
@interface MailMobileMeAccount : MailImapAccount
...
@property (copy) NSURL *runScript;  // If rule matches, run this compiled AppleScript file. Set to empty string to disable this action
