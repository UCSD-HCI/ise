...
int main(int argc, char **argv)
{
  ...
  if (VISCA_set_zoom_value(&iface, &camera, 0x0001)!=VISCA_SUCCESS)
    fprintf(stderr,"error setting zoom 1\n");

  VISCA_usleep(500000);

  if (VISCA_set_zoom_value(&iface, &camera, 0x03FF)!=VISCA_SUCCESS)
    fprintf(stderr,"error setting zoom 2\n");

  VISCA_usleep(500000);

  if (VISCA_set_zoom_value(&iface, &camera, 0x0100)!=VISCA_SUCCESS)
    fprintf(stderr,"error setting zoom 3\n");

  if (VISCA_get_zoom_value(&iface, &camera, &zoom)!=VISCA_SUCCESS)
    fprintf(stderr,"error getting zoom 4\n");
  else
